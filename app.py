...
def index():
    ...
    if request.method == 'POST':
        ...
        if uploaded_file.filename != '':
            # uploaded_file holds the submitted file object
            uploaded_file.save(uploaded_file.filename)
        data_array = process_audio_file(uploaded_file.filename)
        top_3_genres = predict_genres(data_array)
        return render_template('index.html', genres=top_3_genres)
